...
const DEFAULT_DEPENDENCY_BODY = `## 🎯 Goal

Track the August 2026 dependency updates that are intentionally grouped in Dependabot pull requests.

## 📦 Scope

- Review and merge the linked dependency update pull requests.
- Keep unrelated dependency work outside this tracking issue.

## ✅ Acceptance criteria

- Every linked update is reviewed or explicitly deferred.
- The tracking issue is closed only when the August dependency cycle is complete.

## 🔒️ Non-destructive boundary
...
Options:
  ...
  --project-number <number>  Project v2 number (default: 2)
...
export function readSyncConfig(path = DEFAULT_CONFIG_PATH) {
  ...
  return {
    ...
    backfill: {
      ...(config.backfill ?? {}),
      relationMappings: {...(config.backfill?.relationMappings ?? {})},
      dependencyTrackingIssue: {
        ...(config.backfill?.dependencyTrackingIssue ?? {}),
      },
    },
  };
}
...
function configOptions(parsed, config) {
  ...
  const projectNumber = parsed.projectNumber ?? config.project?.number ?? 2;
  return {
    ...parsed,
    repo,
    projectOwner,
...
function dependencyIssueSettings(config) {
  ...
  return {
    title: configured.title ?? 'August 2026 dependency updates',
    variable: configured.variable ?? 'DEPENDENCY_TRACKING_ISSUE',
    ...
    assignees: configured.assignees ?? ['Soku-JINSEOK'],
  };
}
...
function buildRelationOperations({pullRequests, issues, config, selectedPrNumber}) {
  ...
  const configuredDependencyIssueNumber = dependencyIssueNumberFromEnvironment(settings);
  ...
  for (const pullRequest of relevantPullRequests) {
    ...
    operations.push({
      ...
      reason: target === 'dependency-tracking'
        ? 'Dependabot dependency update belongs to the August 2026 tracking Issue.'
        : `Approved historical relation to Issue #${target}.`,
      conflict: null,
      kind: 'pr-relation',
...
function projectOperations({issues, pullRequests, project, config, fileCounts, repo, selectedIssueNumber}) {
  ...
  for (const issue of projectIssues) {
    ...
    if (!item) {
      warnings.push(warning(
        `Issue #${issue.number}`,
        'Issue is not currently a Project #2 item; no Project item will be added.',
        'not-in-project',
      ));
      continue;
    }
...
export async function auditRepository({client, repo, projectOwner = '@me', projectNumber = 2, config, issueNumber, prNumber} = {}) {
...
export async function applyOperations({client, audit, projectOwner = '@me', projectNumber = 2} = {}) {
