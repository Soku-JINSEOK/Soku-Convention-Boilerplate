...
| `docs/standards/SOKU_LIFECYCLE.md` | Both | Reproducible source selection, ownership, non-destructive planning, and rollback protect personal and team repositories alike. |
| `soku/` CLI source and release artifacts | Both | The CLI provides the same portable lifecycle and safety contract at either scale. Its source is maintained in this repository but is distributed through independent `soku/v*` releases, not by the manual sync scripts. |
| `docs/guides/USAGE_MANUAL.md` | Both | This is the human adoption entrypoint. Personal projects can select `bootstrap`; teams can select `standard` or `scaled` without changing lifecycle safety boundaries. |
...
| `templates/_shared/ci/downstream-ci*.yml` | Both | Quick, full, and security CI remain useful even solo. |
...
| `docs/guides/LANGUAGE_SELECTION.md` | Both | The selection criteria and procedure apply solo or on a team; the "organization policy" constraint category and the ADR-style recording template lean Team, but nothing in the document requires one. |
